...
def main():
    ...
    if app_mode == 'About App':
        ...
        st.header("Computer Vision:Introduction to YOLOv8")
        
        
        st.markdown("<style> p{margin: 10px auto; text-align: justify; font-size:20px;}</style>", unsafe_allow_html=True)      
        st.markdown("<p>In this project, we will be exploring the YOLO (You Only Look Once) algorithm. YOLO is known for its ability to detect objects in an image in a single pass, making it a highly efficient and accurate object detection algorithm.🎯</p>", unsafe_allow_html=True)  
        st.markdown("<p>Computer Vision Project🌟</p>", unsafe_allow_html=True)
        ...
        st.markdown("""<p>Submitted to Mam SAMIA KIRAN <br> by Hermen Khan(01-136221-054) <br>
                    Babar Ali(01-136221-051) <br>  Muhammad Taqui(01-136221-021)</p>""", unsafe_allow_html=True)
